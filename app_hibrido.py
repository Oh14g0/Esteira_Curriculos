...
# Configuração MongoDB (apenas para empresas e candidatos)
MONGO_URI = os.getenv('MONGO_URI', 'mongodb://localhost:27017/')
try:
    client = pymongo.MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
    client.server_info()  # Testa a conexão
    db_mongo = client.recrutamento
    print("✅ MongoDB conectado para autenticação!")
except Exception as e:
    print(f"⚠️ MongoDB não disponível: {e}")
    db_mongo = None

# Configuração SQLite (para vagas, candidaturas e notificações)
...
def login_empresa():
    if request.method == 'POST':
        ...
        try:
            if db_mongo:
                empresa = db_mongo.empresas.find_one({'cnpj': cnpj_limpo})
                ...
                flash('Sistema de autenticação indisponível', 'error')
        except Exception as e:
...
def cadastro_empresa():
    if request.method == 'POST':
        ...
        try:
            if db_mongo:
                if db_mongo.empresas.find_one({'cnpj': cnpj_limpo}):
                    flash('CNPJ já está cadastrado no sistema', 'error')
                    return render_template('cadastro_empresa.html')

                if db_mongo.empresas.find_one({'email': email}):
                ...
                db_mongo.empresas.insert_one(empresa_doc)
                flash('Empresa cadastrada com sucesso!', 'success')
                return redirect(url_for('login_empresa'))
            else:
                flash('Sistema de cadastro indisponível', 'error')
        except Exception as e:
...
def login_candidato():
    if request.method == 'POST':
        ...
        if db_mongo:
            candidato = db_mongo.candidatos.find_one({'email': email})
...
def cadastro_candidato():
    if request.method == 'POST':
        ...
        try:
            if db_mongo:
                if db_mongo.candidatos.find_one({'email': email}):
                ...
                db_mongo.candidatos.insert_one(candidato_doc)
...
def candidatos_vaga(vaga_id):
    ...
    try:
        ...
        # Buscar dados dos candidatos no MongoDB
        candidatos = []
        if db_mongo and candidaturas:
            for candidatura in candidaturas:
                candidato = db_mongo.candidatos.find_one({'_id': ObjectId(candidatura['candidato_id'])})
                if candidato:
                    candidatos.append({
...
def dashboard_candidato():
    ...
    try:
        ...
        if db_mongo:
            candidato = db_mongo.candidatos.find_one({'_id': ObjectId(candidato_id)})
        ...
        for vaga in vagas_disponiveis:
            empresa = None
            if db_mongo:
                empresa = db_mongo.empresas.find_one({'_id': ObjectId(vaga['empresa_id'])})
...
def api_candidatos_vaga(vaga_id):
    ...
    try:
        ...
        # Buscar nomes dos candidatos no MongoDB
        candidate_list = []
        if db_mongo and candidaturas:
            for candidatura in candidaturas:
                candidato = db_mongo.candidatos.find_one({'_id': ObjectId(candidatura['candidato_id'])})
...
def detalhes_vaga(vaga_id):
    ...
    try:
        ...
        # Buscar dados da empresa no MongoDB
        empresa = None
        if db_mongo:
            empresa = db_mongo.empresas.find_one({'_id': ObjectId(vaga_data['empresa_id'])})
...
def minhas_candidaturas():
    ...
    try:
        ...
        # Buscar nomes das empresas no MongoDB
        candidaturas = []
        for candidatura in candidaturas_data:
            empresa = None
            if db_mongo:
                empresa = db_mongo.empresas.find_one({'_id': ObjectId(candidatura['empresa_id'])})
...
def baixar_curriculo(candidato_id):
    ...
    try:
        ...
        # Buscar dados do candidato no MongoDB
        if not db_mongo:
            flash('Sistema indisponível', 'error')
            return redirect(url_for('dashboard_empresa'))
...
def api_notificacoes():
    ...
    try:
        ...
        for notificacao in notificacoes_data:
            empresa = None
            if db_mongo:
                empresa = db_mongo.empresas.find_one({'_id': ObjectId(notificacao['empresa_id'])})
